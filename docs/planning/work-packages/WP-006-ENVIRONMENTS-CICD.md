...
**vercel.json is EXPECTED / REQUIRED** (not optional) — its primary purpose is branch deployment eligibility control via Vercel's `git.deploymentEnabled` configuration. S10 creates `vercel.json`; S9 does NOT.
...
**Normal CI DB migration — FORBIDDEN (frozen):** WP-006 introduces NO schema change. Therefore normal CI MUST NOT automatically run `pnpm db:migrate` on every dev push merely as a routine prerequisite. Freeze: NORMAL CI DB MIGRATION = NO. Future migration verification may use a dedicated explicit path when migration files change or when a future WP authorizes it. If a future CI migration job mutates TEST: positive TEST target certification first; same global shared-TEST mutex; never DEV; never Production. Vercel build migration remains FORBIDDEN.

**CI bootstrap:** CI runs `pnpm db:bootstrap` with DATABASE_URL=TEST_DATABASE_URL ONLY for E2E job, AFTER positive TEST target certification (idempotent — creates admin1/fantomas only if missing). This is NOT a normal CI gate — it is specific to the E2E job which needs the identities to exist.
...
**Vitest shared-TEST classification:** The canonical 149 Vitest tests include integration tests that use TEST_DATABASE_URL and mutate Neon TEST (offers-integration, public-offers-integration, auth-regression). These are classified as **STATEFUL TEST** phases. They MUST participate in the same global shared-TEST serialization policy. Do NOT label the complete Vitest suite "non-mutating" without factual proof. PR fast CI may omit stateful Vitest if necessary to remain cheap and isolated.
...
**PR E2E opt-in (label event semantics — frozen):**
- E2E job conditional on `contains(github.event.pull_request.labels.*.name, 'run-e2e')` (per S6 TD-025)
- The workflow MUST react to the `pull_request` `labeled` event type (not merely check a label on an event that never fires when the label is added). The `on: pull_request: types: [labeled]` trigger MUST be included so the workflow fires when the `run-e2e` label is added.
...
- If a PR has the `run-e2e` label, E2E runs on the PR (NON-BLOCKING — informational) AND on push to dev (BLOCKING — canonical integration). This intentional duplication is acceptable only as explicit opt-in + canonical integration, not accidental duplication.
...
**wp/* Preview enforcement (frozen — technically enforceable via git.deploymentEnabled):**
...
- **PRIMARY GUARD: `git.deploymentEnabled`** — Vercel's supported repository configuration provides `git.deploymentEnabled` with branch/glob matching. S10 MUST configure this to disable deployments for `wp/*` branches (and all unapproved branches except `dev`).
- **Ignored Build Step is NOT the security/quota boundary.** Ignored Build Step means SKIP BUILD, not NO DEPLOYMENT EVENT / ZERO VERCEL INVOCATION. It may remain only as an OPTIONAL secondary optimization for eligible deployments (e.g., docs-only changes on `dev`), but it MUST NOT be used as proof that a branch causes zero Vercel deployment.
- Acceptance criteria MUST prove: push to `wp/006-environments-cicd` does NOT create a Vercel deployment event.
- Acceptance criteria MUST prove: future `wp/*` branches do NOT automatically create deployments.
- No Vercel runtime should be consumed by normal WP pushes.
- S10 MUST verify actual Vercel glob/config behavior for `wp/006-*`, `wp/*`, `dev`, `main`, and an arbitrary unrelated branch using official Vercel config/schema behavior.
- If a deny-by-default branch configuration cannot be safely expressed: **STOP** (`VERCEL_BRANCH_DEPLOYMENT_FILTER_GAP`).

**STOP CONDITION:** `WP_BRANCH_PREVIEW_SUPPRESSION_GAP` — if Vercel cannot provide the required branch deployment suppression cleanly via `git.deploymentEnabled` or equivalent, STOP.
...
The intended semantic policy for `git.deploymentEnabled` (frozen):
- `dev`: deployment ENABLED (the ONLY approved automatic Preview-eligible Git branch during MS-006)
- `main`: deployment DISABLED during MS-006 / pre-S13
- `wp/*`: deployment DISABLED
- All other unapproved branches: deployment DISABLED by default where safely expressible
- Ignored Build Step may remain as OPTIONAL secondary optimization for eligible deployments (e.g., docs-only changes on `dev`), but MUST NOT be used as the branch security/quota boundary.

**Branch-to-deployment mapping (frozen):**

| Git Branch | Vercel Environment | Neon Branch | Auto-Deploy? | Vercel Invocations |
|---|---|---|---|---|
| `wp/*` (work branches) | NO Vercel deployment | NO Neon branch | NO — `git.deploymentEnabled: false` for `wp/*` | **0** |
...
- NO wp/* deployments (`git.deploymentEnabled: false` for `wp/*`)
...
|---|---|---|---|
| TEST | **NOT in normal CI** — WP-006 introduces no schema change. Future migration verification: dedicated explicit path when migration files change, using the global shared-TEST mutex, AFTER positive TEST target certification. | CI (future dedicated job) or operator | `drizzle-kit migrate` (forward migrations only; versioned) |
...
- **Normal CI does NOT automatically migrate TEST schema.** WP-006 introduces no schema change. Future migration verification: dedicated explicit path when migration files change, using the global shared-TEST mutex, AFTER positive TEST target certification.
...
| wp/* branch push | N/A (no deployment) | N/A | 0 | NO | **0** (`git.deploymentEnabled: false`) | Every wp/* push | N/A | N/A |
| arbitrary unapproved branch push | N/A (no deployment) | N/A | 0 | NO | **0** (`git.deploymentEnabled: false` deny-by-default) | Every unapproved branch push | N/A | N/A |
...
**A. Non-mutating CI (lint, typecheck, pure build, pure static checks):**
...
**B. ALL stateful shared-TEST phases (Vitest integration, Playwright E2E, db:migrate if future, db:bootstrap for E2E, rateLimit cleanup, fixture creation/cleanup):**
- `concurrency: group: jourdain-shared-test-stateful, cancel-in-progress: false` — ONE GLOBAL MUTEX across ALL branches and events
- At most ONE stateful job using shared TEST at a time
- NO parallel stateful TEST across separate PRs
- NO parallel PR/dev/main/manual stateful TEST
- NO two jobs clearing rateLimit simultaneously
- NO two bootstrap/fixture lifecycles overlapping
- NO two migration operations overlapping
- `cancel-in-progress: false` — a running stateful job is NOT arbitrarily killed mid-mutation (queued jobs wait for the mutex)
- Timeout: 15 min per stateful job (prevents indefinite lock)
- This group covers: Vitest integration tests (stateful), Playwright E2E (stateful), `pnpm db:migrate` (if future migration job), `pnpm db:bootstrap` (for E2E), rateLimit cleanup, fixture creation/cleanup

**Cancel-in-progress:** YES for non-mutating CI; NO for all stateful shared-TEST phases (per above)

**Timeouts:**
- CI workflow: 30 min total
- CI jobs: 5-15 min per job (see §5.2)
- E2E job: 15 min (also serves as mutex timeout for stateful shared-TEST phases)
...
- wp/* branches: NO Vercel deployment (`git.deploymentEnabled: false` for `wp/*` — no deployment event at all)
...
- CI stateful TEST job (Vitest integration + E2E) runs only on: push to dev + `run-e2e` label on PR + `workflow_dispatch`
- Global mutex (`jourdain-shared-test-stateful`) ensures only one stateful shared-TEST job at a time across ALL branches
...
- No separate E2E workflow (single `ci.yml` with conditional stateful TEST jobs using global mutex concurrency)
...
| 7 | S10 start | Vercel dashboard → Settings → Git (or `vercel.json`) | **Configure `git.deploymentEnabled` for branch deployment eligibility**: enable deployment for `dev` ONLY; disable for `wp/*`, `main` (pre-S13), and all other unapproved branches. Ignored Build Step is NOT the security boundary — it is an optional secondary optimization only. | NO | YES (confirm `git.deploymentEnabled` config; verify wp/* and main do NOT deploy) |
...
jobs:
  ...
  test:
    ...
    steps:
      ...
      - setup pnpm
      - pnpm install --frozen-lockfile
      - pnpm test  # Vitest (149 tests) — stateful: uses shared-TEST mutex (jourdain-shared-test-stateful)
...
**No `vercel.json` implementation during S9.** S10 creates `vercel.json` as EXPECTED / REQUIRED. Its primary purpose is branch deployment eligibility control via Vercel's `git.deploymentEnabled` configuration. S10 may also include framework preset, build command, or install command if not auto-detected by Vercel.

**vercel.json MUST NOT contain:** secrets, DB migration commands, bootstrap commands, or remote E2E commands. Only deployment eligibility + build configuration.
...
| AC-022 | Normal CI does NOT automatically run `pnpm db:migrate` on every push (WP-006 introduces no schema change; future migration verification uses dedicated explicit path with shared-TEST mutex + TEST target certification) |
...
| AC-034 | wp/* branches do NOT get Vercel deployments (`git.deploymentEnabled: false` for `wp/*` — quota-safe: true deployment-disable, NOT merely Ignored Build Step skip-build) |
...
| AC-037 | No automatic per-PR Neon branch auto-provisioning (removed; dev Preview uses existing Neon dev branch) |
| AC-038 | Preview DATABASE_URL = Neon dev branch (DEV_DATABASE_URL, NOT Neon main / Production, NOT auto-provisioned preview branch) |
...
| AC ID | Criterion |
|---|---|
| AC-060 | Normal CI does NOT automatically migrate TEST schema (WP-006 introduces no schema change); future migration verification: dedicated explicit path with shared-TEST mutex + TEST certification |
| AC-061 | CI does NOT apply migrations to DEV or PRODUCTION |
| AC-062 | Vercel Preview build does NOT execute migrations (Vercel build command = `pnpm build` only; NO `pnpm db:migrate`) |
...
| AC ID | Criterion |
|---|---|
| AC-110 | wp/* push does NOT create a Vercel deployment event (`git.deploymentEnabled: false` for `wp/*` — NOT merely Ignored Build Step skip-build) |
...
| AC-117 | ALL stateful shared-TEST CI phases are globally serialized (concurrency group `jourdain-shared-test-stateful`, `cancel-in-progress: false`, max 1 parallel — covers Vitest integration + E2E + db:migrate + db:bootstrap + rateLimit cleanup + fixture creation/cleanup) |
| AC-118 | CI push-to-dev coverage exists (push to dev triggers lint + typecheck + Vitest + build + E2E) |
| AC-119 | Full E2E does NOT run redundantly on every event (only on push to dev + opt-in `run-e2e` label via `labeled` event + workflow_dispatch; NOT on every PR or push to main) |
...
| Migrations on TEST | Verify CI does NOT automatically migrate TEST in normal pipeline (WP-006 introduces no schema change); future migration verification: dedicated explicit path with shared-TEST mutex + TEST certification |
...
| `vercel.json` | CREATED (EXPECTED / REQUIRED) | Vercel project configuration: `git.deploymentEnabled` branch eligibility control (PRIMARY quota-safety guard), framework preset, build command, install command. NO secrets, NO migration commands, NO bootstrap commands. |
...
| `vercel.json` | NOT created (S10 will create — EXPECTED / REQUIRED for `git.deploymentEnabled` branch eligibility control) |
